...
while currTrial < TRIALS:
    ...
    available_columns = newdf.columns.tolist()
    for i in range (len(available_columns)):
        if(coefficients[i] != 0):
            selected_columns.append(available_columns[i])

    X_train_selected = X_train[selected_columns]
    X_test_selected = X_test[selected_columns]

    model = LogisticRegressionCV( #log reg
    cv=5,                  # 5-fold cross-validation
    scoring='f1',          # Optimize for F1 score (can also use 'accuracy', 'roc_auc', etc.)
    penalty='l2',          # Or 'l1' with solver='liblinear'
    solver='liblinear',    # Supports both L1 and L2
    class_weight='balanced',  # Handle class imbalance
    random_state=42
    )
    model.fit(X_train_selected,y_train)
    y_pred_bin = model.predict(X_test_selected)

    '''neg, pos = np.bincount(y_train) #xgboost
    ...
    y_pred_bin = model.predict(X_test_selected) #end xgboost'''
