...
def cross_validate_model(features, response, k, folds, standardize, seed,gamma=1,sampleWeights=None):
    """
    Train an L0-Regression for each fold and report the cross-validated MSE.
    """
    if seed is not None:
    ...
    for fold in range(folds):
        ...
        if standardize:
            ...
            xtest = scaler.transform(xtest)
        if sampleWeights is not None:
            equation = gurobiSVM(xtrain, ytrain,k/2,gamma=gamma,M=1000,L0Regularization=True,sampleWeights=sampleWeights)
        else:
            equation = gurobiSVM(xtrain, ytrain,k/2,gamma=gamma,M=1000,L0Regularization=True)
        ypred = findYPred(xtest,equation)
        newroc = roc_auc_score(ytest, ypred)
        if newroc > bestroc:
            bestroc = newroc
