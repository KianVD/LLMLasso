...
def NarrowDownDFLLM(df,contextFilePath, featuresToGet):
    ...
    valid_cols = valid_cols[:featuresToGet] #cut off any extra columns if llm included too many TODO how to validate there are exactly featuresToGet columns returned
...
for model in ["LLMBSS","BSS"]:
    ...
    for feature in FEATURES:
        ...
        while currTrial < TRIALS:
            ...
            if model == "LLMBSS":
                newdf = NarrowDownDFLLM(df,"Spotify/contextSpotify.txt",feature) #here is where you specify how many features the LLM should choose
            else:
                newdf = df
            ...
            intercept, coefficients = L0_regression(X_train_std,y_train.to_numpy(),feature,standardize=True,seed=currTrial) #set seed and 15 as max k 
        ...
        outputdf.to_csv(f'output{model}p{feature}.csv', index=True)